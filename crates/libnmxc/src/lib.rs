...
impl NmxcTlsConfig {
    ...
    fn material_newer_than(&self, created: SystemTime) -> bool {
        [
            &self.ca_cert_path,
...
trait ChannelConnector: Send + Sync + std::fmt::Debug {
    ...
    fn is_stale(&self, created: SystemTime) -> bool;
}
...
impl NmxcClientPool {
    ...
    async fn channel_for(&self, endpoint: &Endpoint, uri: &str) -> Result<Channel, NmxcError> {
        ...
        if let Some((channel, created)) = cached {
            if !self.connector.is_stale(created) {
                return Ok(channel);
            }
...
impl ChannelConnector for TlsChannelConnector {
    ...
    fn is_stale(&self, created: SystemTime) -> bool {
        ...
        self.tls
            .as_ref()
            .is_some_and(|tls| tls.material_newer_than(created))
    }
}
